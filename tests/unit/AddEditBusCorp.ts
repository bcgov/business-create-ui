...
// Events
const addEditPersonEvent: string = 'AddEditBusCorp'
 ...
 */
function getLastEvent (wrapper: Wrapper<AddEditBusCorp>, name: string): any {
 ...
 *
 * @returns a Wrapper<AddEditBusCorp> object with the given parameters.
 */
...
): Wrapper<AddEditBusCorp> {
  const localVue = createLocalVue()
  localVue.use(Vuetify)
  document.body.setAttribute('data-app', 'true')
  return mount(AddEditBusCorp, {
    localVue,
    propsData: {
      initialValue,
      activeIndex,
...
describe('Add/Edit Bus/Corp component', () => {
  it('Loads the component and sets data for person', async () => {
    const wrapper: Wrapper<AddEditBusCorp> = createComponent(validPersonData, -1, null)
    expect(wrapper.vm.$data.AddEditBusCorp).toStrictEqual(validPersonData)
  ...
  it('Loads the component and sets data for org', async () => {
    const wrapper: Wrapper<AddEditBusCorp> = createComponent(validOrgData, -1, null)
    expect(wrapper.vm.$data.AddEditBusCorp).toStrictEqual(validOrgData)
  ...
  it('Displays form data for person', async () => {
    const wrapper: Wrapper<AddEditBusCorp> = createComponent(validPersonData, 0, null)
  ...
  it('Remove button is enabled in edit mode', async () => {
    const wrapper: Wrapper<AddEditBusCorp> = createComponent(validOrgData, 0, null)
    expect(wrapper.find(removeButtonSelector).attributes('disabled')).toBeUndefined()
    wrapper.destroy()
  })
  ...
  it('Shows popup if there is already a completing party', async () => {
    ...
    const wrapper: Wrapper<AddEditBusCorp> = createComponent(validIncorporator, -1, validPersonData)
  ...
  it('Emits events correctly on confirming reassign completing party', async () => {
    ...
    expect(wrapper.emitted().AddEditBusCorp).toBeTruthy()
