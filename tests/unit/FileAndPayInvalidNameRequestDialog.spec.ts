...
import { FileAndPayInvalidNameRequestDialog } from '@/components/dialogs'
Vue.use(Vuetify)
...
describe('FileAndPayInvalidNameRequestDialog - Verify that dialog is displayed correctly', () => {
  it('displays dialog with the proper store data', async () => {
    ...
    expect(wrapper.findAll('div > div').at(0).text()).toContain('IMPORTANT:')
    expect(wrapper.findAll('div > div').at(1).text()).toContain('Once the reservation period for a Name Request ' +
    'expires or is otherwise cancelled, that name becomes')
    expect(wrapper.findAll('div > div').at(1).text()).toContain('available to anyone wishing to start their business ' +
    'with that name.')
