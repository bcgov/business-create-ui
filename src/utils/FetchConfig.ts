...
export async function FetchConfig (): Promise<any> {
  ...
  const processEnvVueAppPath: string = import.meta.env.VITE_PATH
  ...
  const authWebUrl: string = import.meta.env.VITE_AUTH_WEB_URL
  sessionStorage.setItem('AUTH_WEB_URL', authWebUrl)

  const registryHomeUrl: string = import.meta.env.VITE_REGISTRY_HOME_URL
  sessionStorage.setItem('REGISTRY_HOME_URL', registryHomeUrl)

  const businessesUrl: string = import.meta.env.VITE_BUSINESSES_URL
  sessionStorage.setItem('BUSINESSES_URL', businessesUrl)

  const dashboardUrl: string = import.meta.env.VITE_DASHBOARD_URL
  sessionStorage.setItem('DASHBOARD_URL', dashboardUrl)

  const legalApiUrl: string = (import.meta.env.VITE_LEGAL_API_URL + import.meta.env.VITE_LEGAL_API_VERSION_2 + '/')
  // set base URL for axios calls
  axios.defaults.baseURL = legalApiUrl

  const naicsUrl: string = (import.meta.env.VITE_NAICS_API_URL + import.meta.env.VITE_NAICS_API_VERSION_2 + '/')
  sessionStorage.setItem('NAICS_URL', naicsUrl)

  const registriesSearchApiUrl: string =
    (import.meta.env.VITE_REGISTRIES_SEARCH_API_URL + import.meta.env.VITE_REGISTRIES_SEARCH_API_VERSION + '/')
  sessionStorage.setItem('REGISTRIES_SEARCH_API_URL', registriesSearchApiUrl)

  const businessApiKey: string = import.meta.env.VITE_REGISTRIES_SEARCH_API_KEY
  sessionStorage.setItem('BUSINESS_API_KEY', businessApiKey)

  const authApiUrl: string = (import.meta.env.VITE_AUTH_API_URL + import.meta.env.VITE_AUTH_API_VERSION + '/')
  sessionStorage.setItem('AUTH_API_URL', authApiUrl)

  const payApiUrl: string = (import.meta.env.VITE_PAY_API_URL + import.meta.env.VITE_PAY_API_VERSION + '/')
  sessionStorage.setItem('PAY_API_URL', payApiUrl)

  // for system alert banner (sbc-common-components)
  const statusApiUrl: string = (import.meta.env.VITE_STATUS_API_URL + import.meta.env.VITE_STATUS_API_VERSION)
  sessionStorage.setItem('STATUS_API_URL', statusApiUrl)

  const siteminderLogoutUrl: string = import.meta.env.VITE_SITEMINDER_LOGOUT_URL
  if (siteminderLogoutUrl) {
    sessionStorage.setItem('SITEMINDER_LOGOUT_URL', siteminderLogoutUrl)
  }

  const keycloakAuthUrl: string = import.meta.env.VITE_KEYCLOAK_AUTH_URL;
  ...
  const keycloakClientId: string = import.meta.env.VITE_KEYCLOAK_CLIENTID;
  (<any>window).keycloakClientId = keycloakClientId

  const iaSurveyId: string = import.meta.env.VITE_IA_SURVEY_ID
  // NB: set empty string if iaSurveyId is falsy (undefined, null or 0)
  sessionStorage.setItem('IA_SURVEY_ID', iaSurveyId || '')

  const hotjarId: string = import.meta.env.VITE_HOTJAR_ID;
  (<any>window).hotjarId = hotjarId

  const addressCompleteKey: string = import.meta.env.VITE_ADDRESS_COMPLETE_KEY;
  (<any>window).addressCompleteKey = addressCompleteKey

  const ldClientId: string = import.meta.env.VITE_BUSINESS_CREATE_LD_CLIENT_ID;
  (<any>window).ldClientId = ldClientId

  const sentryDsn: string = import.meta.env.VITE_SENTRY_DSN;
  (<any>window).sentryDsn = sentryDsn

  const genesysEnv: string = import.meta.env.VITE_GENESYS_ENV;
  (<any>window).genesysEnv = genesysEnv

  const genesysId: string = import.meta.env.VITE_GENESYS_ID;
  (<any>window).genesysId = genesysId

  const genesysUrl: string = import.meta.env.VITE_GENESYS_URL;
  (<any>window).genesysUrl = genesysUrl

  const webChatReason: string = import.meta.env.VITE_WEBCHAT_REASON;
  (<any>window).webChatReason = webChatReason

  const webChatStatusUrl: string = import.meta.env.VITE_WEBCHAT_STATUS_URL;
  (<any>window).webChatStatusUrl = webChatStatusUrl
  if (webChatStatusUrl) console.info('Set WebChat Status URL to: ' + webChatStatusUrl)

  const webChatUrl: string = import.meta.env.VITE_WEBCHAT_URL;
